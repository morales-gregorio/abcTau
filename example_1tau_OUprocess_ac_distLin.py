...
max_lag = 500 # only used when suing autocorrelation for summary statistics
...
class my_model(abcTau.Model):
    ...
    def generate_data(self, theta):
        ...
        syn_summary = abcTau.summary_stats.comp_summary(syn_data, summary_metric, normalize, \
                                                        dt, binsize, T, n_bin_data, max_lag)
        return syn_summary
...
abc_results, final_step = abcTau.fit.fit_withABC(my_model, data_summary, prior_dist,
                                                 inter_save_direc, inter_filename,
                                                 datasave_path, filename_save, epsilon_0,
                                                 min_samples, steps, min_acc_rate,
